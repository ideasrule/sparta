...
def correct_lc(wavelengths, fluxes, bjds, t0, t_secondary, per, rp, a, inc,
    ...
    if extra_phase_terms:
        initial_params = np.array([fp, C1, D1, C2, D2, rp, get_mad(fluxes), 1])
    else:
        initial_params = np.array([fp, C1, D1, rp, get_mad(fluxes), 1])
...
correct_lc(wavelengths, binned_fluxes, binned_bjds, float(items["t0"]), float(items["t_secondary"]), float(items["per"]),
