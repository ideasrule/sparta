...
def get_mask():
    with astropy.io.fits.open(MASK_FILE) as hdul:
        mask = np.asarray(hdul["DQ"].data[SLITLESS_TOP:SLITLESS_BOT, SLITLESS_LEFT:SLITLESS_RIGHT])
    return mask
...
'''def apply_nonlinearity(data):   
    ...
    result = data + c2 * data**2 + c3 * data**3 + c4 * data**4
    end = time.time()
    print("Non linearity took", end - start)
...
for filename in sys.argv[1:]:
    ...
    sci_hdu = astropy.io.fits.ImageHDU(np.cpu(final_signal), name="SCI")
    err_hdu = astropy.io.fits.ImageHDU(np.cpu(final_error), name="ERR")
