...
def correct_lc(wavelengths, fluxes, errors, bjds, y, x, t0, per, rp, a, inc,
    ...
    initial_params = np.array([rp**2, error_factor, 1, 0, 1./24, 0, 0, 0])
    labels = ["rp^2", "error", "Fstar", "Aramp", "tau", "cy", "cx", "m"]
...
chain, lnprobs = correct_lc(wavelengths, binned_fluxes, binned_errors, binned_bjds, binned_y - smoothed_binned_y, binned_x - smoothed_binned_x, float(items["t0"]), float(items["per"]),
