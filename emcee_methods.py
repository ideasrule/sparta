...
def plot_fit_and_residuals(phases, binsize, Lobserved, Lexpected, star_variation, gp=None, bjds=None):
    ...
    axarr[1].scatter(bin_data(phases,binsize), bin_data(Lobserved - star_variation, binsize), s=10, c='blue', edgecolors='black')
    axarr[1].plot(bin_data(phases,binsize), bin_data(Lexpected - star_variation,binsize), color="r")
    ...
    axarr[2].scatter(bin_data(phases,binsize), bin_data(residuals,binsize), s=10, c='blue', edgecolors='black')
    axarr[2].set_ylabel("Relative flux", fontsize=fontsize)
    


#@profile    
def lnprob_with_time_slope(params, initial_batman_params, transit_model, eclipse_model, bjds,
                           fluxes, initial_t0, extra_phase_terms=False, plot_result=False, max_Fp=1,
    ...
    error = params[end_phase_terms + 3]
    ...
    result = -0.5*(np.sum(residuals**2/error**2 - np.log(1.0/error**2)))


    '''if result/len(residuals) > 5.28:
        plt.scatter(bjds, Lobserved)
        plt.plot(bjds, Lexpected, color='r')
                
        plt.figure()
        plt.scatter(bjds, residuals)
        plt.show()'''
